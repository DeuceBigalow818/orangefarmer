...
const CONTRACTS = {
  ...
  testnet: {
    // NOTE: No DeFi contracts deployed on testnet (verified via btc_getCode)
    // Only RPC is available for block/fee monitoring
    RPC: "https://testnet.opnet.org",
  },
  mainnet: {
    // NOTE: No DeFi contracts deployed on mainnet yet
...
export default function OrangeFarmer() {
  ...
  // ── Real unstake (unstakes ALL — STAKING_ABI unstake() takes no args) ──
  const realUnstake = useCallback(async () => {
    if (!walletConnected || !walletAddr || txPending) return;
    setTxPending(true);
    setTxTarget("Unstake");
    try {
      addLog(`[Withdraw] Unstaking all MOTO...`, "info");
      const receipt = await SDK.unstakeMoto(network, 0, walletAddr, (s) => setTxStatus(s));
          ...
          {tab === "dashboard" && <>
            ...
            {walletConnected && (
              <div style={{ display:"flex", gap:8, flexWrap:"wrap", alignItems:"center" }}>
                <button onClick={realClaimRewards} disabled={txPending || !sdkAvailable} style={{ padding:"8px 18px", background: sdkAvailable ? "linear-gradient(135deg, #00e676, #00c853)" : "#333", border:"none", borderRadius:8, color: sdkAvailable ? "#000" : "#666", fontWeight:700, fontSize:12, cursor: sdkAvailable ? "pointer" : "not-allowed", opacity:txPending?0.5:1 }}>
                  Claim Rewards
                </button>
                <button onClick={() => realUnstake()} disabled={txPending || !sdkAvailable} style={{ padding:"8px 18px", background:"none", border: sdkAvailable ? "1px solid #ff9800" : "1px solid #333", borderRadius:8, color: sdkAvailable ? "#ff9800" : "#666", fontWeight:600, fontSize:12, cursor: sdkAvailable ? "pointer" : "not-allowed", opacity:txPending?0.5:1 }}>
                  Unstake All
                </button>
                ...
                {!sdkAvailable && <span style={{ fontSize:11, color:"#ff3d00", alignSelf:"center" }}>SDK not loaded — simulation mode only</span>}
                {sdkAvailable && !SDK.getNetworkConfig(network)?.staking && <span style={{ fontSize:11, color:"#ff9800", alignSelf:"center" }}>Staking not deployed on {network}</span>}
              </div>
            )}
                        ...
                        {eligible ? (
                          <div style={{ display:"flex", gap:4, alignItems:"center" }}>
                            ...
                            {p.stake > 0 && sdkAvailable && p.proto === "Staking" && (
                              <button onClick={() => realUnstake()} disabled={txPending} style={{ padding:"4px 10px", background:"none", border:"1px solid #ff980060", borderRadius:6, color:"#ff9800", fontSize:10, cursor:"pointer", opacity:txPending?0.5:1 }}>Withdraw All</button>
                            )}
