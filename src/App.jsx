...
const CONTRACTS = {
  ...
  testnet: {
    // Verified 2026-03-04: NO DeFi contracts deployed on testnet
    RPC: "https://testnet.opnet.org",
  },
  mainnet: {
    // TODO: fill when mainnet launches
    RPC: "https://mainnet.opnet.org",
  },
};

const POOLS = [
  { id: 1, name: "UNGA / MOTO", t0: "UNGA", t1: "MOTO", apr: 127.4, tvl: 2450000, vol: 580000, stake: 0, rew: 0, risk: "medium", proto: "MotoSwap", nets: ["regtest"] },
  { id: 2, name: "BTC / MOTO", t0: "BTC", t1: "MOTO", apr: 89.2, tvl: 5200000, vol: 1200000, stake: 0, rew: 0, risk: "low", proto: "NativeSwap", nets: ["regtest"] },
  { id: 3, name: "MOTO / PILL", t0: "MOTO", t1: "PILL", apr: 245.8, tvl: 890000, vol: 320000, stake: 0, rew: 0, risk: "high", proto: "MotoSwap", nets: ["regtest"] },
  { id: 4, name: "PEPE / MOTO", t0: "PEPE", t1: "MOTO", apr: 156.3, tvl: 1750000, vol: 410000, stake: 0, rew: 0, risk: "medium", proto: "MotoSwap", nets: ["regtest"] },
  { id: 5, name: "BTC / PILL", t0: "BTC", t1: "PILL", apr: 312.5, tvl: 430000, vol: 190000, stake: 0, rew: 0, risk: "high", proto: "NativeSwap", nets: ["regtest"] },
  { id: 6, name: "MOTO Staking", t0: "MOTO", t1: "", apr: 45.6, tvl: 8900000, vol: 0, stake: 0, rew: 0, risk: "low", proto: "Staking", nets: ["regtest"] },
  ...
  { id: 10, name: "TARD-MOTO Farm", t0: "TARD", t1: "MOTO", apr: 185.7, tvl: 2100000, vol: 610000, stake: 0, rew: 0, risk: "medium", proto: "MotoChef", nets: ["regtest"] },
...
const poolAvailable = (pool, net) => true; // All pools are simulation-available; real network gating happens in SDK
...
// ── Profitability Engine — gas-vs-yield checks for 10min BTC blocks ──
// Estimated transaction sizes in vbytes (OPNet contract calls)
const TX_VBYTES = {
  stake:       250,  // approve + stake = ~250 vB
  unstake:     180,  // single contract call
  claim:       180,  // claimRewards()
  compound:    350,  // claim + approve + restake
  rotate:      400,  // withdraw from A + approve + deposit to B
  addLP:       350,  // approve both tokens + addLiquidity
  rebalance:   300,  // per-pool adjustment
  nativeSwap:  280,  // reserve + swap (2 txs)
};

// Minimum profit multiplier — only execute if yield > gas * this factor
// 2.0 = action must earn at least 2x its gas cost
const MIN_PROFIT_RATIO = 2.0;

// Minimum APR gap (%) to justify pool rotation — prevents churn
const MIN_ROTATION_APR_GAP = 15;

// Minimum accumulated rewards (MOTO) to justify a harvest transaction
// Below this, wait for more rewards to accumulate
const MIN_HARVEST_MOTO = 5;

// Assumed MOTO price in sats for profitability calc (updated from pool data in production)
const MOTO_PRICE_SATS = 500; // 1 MOTO ≈ 500 sats (placeholder)

// Estimate gas cost of a transaction in sats
const estimateGasCost = (action, feeRateSatVb) => {
  const vbytes = TX_VBYTES[action] || 250;
  return Math.ceil(vbytes * feeRateSatVb);
};

// Check if an action is profitable: expected yield (sats) vs gas cost (sats)
const isProfitable = (action, expectedYieldMoto, feeRateSatVb) => {
  const gasCost = estimateGasCost(action, feeRateSatVb);
  const yieldSats = expectedYieldMoto * MOTO_PRICE_SATS;
  const ratio = gasCost > 0 ? yieldSats / gasCost : Infinity;
  return { profitable: ratio >= MIN_PROFIT_RATIO, ratio, gasCost, yieldSats };
};

// Estimate yield since last action for a staked position (MOTO per 30s cycle)
const estimateYieldPerCycle = (stakeAmount, aprPercent) => {
  // APR → per-30-second yield
  // yield = stake * (apr/100) / (365 * 24 * 60 * 2)  [2 cycles per minute]
  return stakeAmount * (aprPercent / 100) / (365 * 24 * 120);
};
...
export default function OrangeFarmer() {
  ...
  const [confirmDeposits, setConfirmDeposits] = useState(true); // require confirmation modal before deposits
  const [botConfirmModal, setBotConfirmModal] = useState(false); // show confirm before starting bot
  ...
  const [confirmModal, setConfirmModal] = useState(null); // null or { pool, depositType, gasCost, dailyYield, isSim }
  ...
  const sendNotification = useCallback(async (event, message) => {
    // Check if this event type is enabled
    if (!notifEvents[event]) return;

    // Telegram
    if (tgConnected && notifTg) {
      try {
        const parts = notifTg.split("/").filter(Boolean);
        if (parts.length >= 2) {
          const token = parts[0];
          const chatId = parts.slice(1).join("/");
          const text = `🚜 *OrangeFarmer*\n${message}`;
        ...
        console.warn("[Notif] Telegram send failed:", e);
      }
    }

    // Discord
    if (dcConnected && notifDc) {
      try {
        await fetch(notifDc, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ content: `🚜 **OrangeFarmer** — ${message}` }),
        });
      } catch (e) {
        console.warn("[Notif] Discord send failed:", e);
      }
    }
  }, [tgConnected, dcConnected, notifTg, notifDc, notifEvents]);
  ...
  const connectWallet = useCallback(async () => {
    ...
    try {
      ...
      // Detect wallet network (informational only — does NOT auto-switch)
      let networkName = "";
      try {
        const net = await w.provider.getNetwork();
        networkName = net || "";
        // Log detected network but keep user's manual selection
      } catch (_) {}

      addLog(`Wallet connected via ${w.name}: ${address.slice(0,12)}...${address.slice(-8)}${networkName ? ` (wallet on ${networkName})` : ""} | App network: ${network}`, "success");

      // Initialize pool positions (simulated — real positions would come from RPC)
      setPools(prev => prev.map((p, i) => ({
        ...p,
        stake: poolAvailable(p, network) && i < 3 ? parseFloat((rand(posSize * 0.5, posSize * 1.5)).toFixed(8)) : 0,
        rew: poolAvailable(p, network) && i < 3 ? parseFloat(rand(0.0001, 0.005).toFixed(8)) : 0,
      })));
      ...
      try {
        ...
        w.provider.on("networkChanged", (net) => {
          addLog(`Wallet network changed to: ${net} (app stays on ${network} — switch manually if needed)`, "info");
        });
  ...
  useEffect(() => {
    import("./opnet-sdk.js")
      .then(async (mod) => {
        SDK = mod;
        const avail = await SDK.isSDKAvailable();
        setSdkAvailable(avail);
        addLog(avail ? "OPNet SDK loaded - real transactions enabled" : "OPNet SDK loaded but opnet package unavailable", avail ? "success" : "warning");
      })
      .catch((err) => {
        console.warn("OPNet SDK load failed, simulation mode:", err);
        setSdkAvailable(false);
        addLog("OPNet SDK unavailable - simulation mode: " + err.message, "warning");
      });
  }, [addLog]);
  ...
  // ── Deposit confirmation — shows modal with tx details before executing ──
  const confirmDeposit = useCallback((pool) => {
    const isSim = !sdkAvailable || !walletConnected;
    const depositType = SDK.getPoolDepositType(pool);
    const effectiveFee = Math.max(feeRate, 1);
    const txType = depositType === "staking" ? "stake" : depositType === "nativeswap" ? "nativeSwap" : "addLP";
    const gasCost = estimateGasCost(txType, effectiveFee);
    const dailyYield = posSize * (pool.apr / 100) / 365;
    const dailyYieldSats = dailyYield * MOTO_PRICE_SATS;
    const daysToPaybackGas = gasCost > 0 && dailyYieldSats > 0 ? gasCost / dailyYieldSats : 0;
    ...
    setConfirmModal({
      ...
      dailyYieldSats,
      daysToPaybackGas,
      feeRateUsed: effectiveFee,
      network,
      walletAddr: walletAddr || "(not connected)",
    });
  }, [sdkAvailable, walletConnected, feeRate, posSize, network, walletAddr]);

  // realDepositRef lets executeConfirmedDeposit call realDeposit even though it's defined later
  const realDepositRef = useRef(null);

  // ── Execute deposit after confirmation ──
  const executeConfirmedDeposit = useCallback(() => {
    if (!confirmModal) return;
    const { pool, isSim } = confirmModal;
    setConfirmModal(null);
    if (isSim) {
      setPools(prev => prev.map(p => p.id === pool.id ? { ...p, stake: p.stake + posSize } : p));
      addLog(`[Sim] Deposited ${fmtBtc(posSize)} into ${pool.name}`, "success");
    } else if (realDepositRef.current) {
      realDepositRef.current(pool);
    }
  }, [confirmModal, posSize, addLog]);
  ...
  const realDeposit = useCallback(async (pool) => {
    ...
    try {
      if (depositType === "staking") {
        ...
        // WBTC pairs or unsupported — simulate with warning
        addLog(`[Deposit] ${pool.name}: ${depositType === "wbtc-pending" ? "WBTC contract not yet deployed" : "Unsupported pool type"}. Simulating deposit.`, "warning");
        setPools(prev => prev.map(p => p.id === pool.id ? { ...p, stake: p.stake + posSize } : p));
      }
      ...
      sendNotification("error", `Deposit failed: ${msg}`);
      setTxStatus("error");
    } finally {
      setTxPending(false);
  ...
  realDepositRef.current = realDeposit;
  ...
  // ── Real unstake — MOTOSWAP_STAKING_ABI: unstake(amount) ──
  ...
  useEffect(() => {
    ...
    const runCycle = async () => {
      ...
      // 4. Execute strategy actions with profitability checks
      const actionsPerCycle = strategy === "aggressive" ? 4 : strategy === "balanced" ? 3 : 2;
      const effectiveFeeRate = Math.max(feeRate, 1); // floor at 1 sat/vB for calc
      ...
      for (let a = 0; a < actionsPerCycle; a++) {
        ...
        if (act === "Auto-compound" && autoCompound && staked.length > 0) {
          ...
          // Profitability check: is compounding worth the gas?
          // Compound yield = accumulated rewards that will start earning their own APR
          const pendingRewards = target.rew;
          const compoundCheck = isProfitable("compound", pendingRewards, effectiveFeeRate);
          if (!compoundCheck.profitable) {
            addLog(`[${act}] SKIP ${target.name} — gas: ${compoundCheck.gasCost} sats > yield value: ${Math.floor(compoundCheck.yieldSats)} sats (ratio: ${compoundCheck.ratio.toFixed(1)}x, need ${MIN_PROFIT_RATIO}x)`, "warning");
            continue;
          }
          const amount = rand(0.5, strategy === "aggressive" ? 12 : 5);
          addLog(`[${act}] Compounding ${target.name} -> +${amount.toFixed(2)} MOTO | Gas: ~${compoundCheck.gasCost} sats | Profit ratio: ${compoundCheck.ratio.toFixed(1)}x`, "success");

        } else if (act === "Pool rotation" && eligible.length > 1) {
          if (shouldSkipExpensive) continue;
          const sorted = [...eligible].sort((a, b) => b.apr - a.apr);
          const best = sorted[0], worst = sorted[sorted.length - 1];
          const aprGap = best.apr - worst.apr;
          if (best.id !== worst.id && worst.stake > 0) {
            // Check 1: Is the APR gap large enough to justify rotation?
            if (aprGap < MIN_ROTATION_APR_GAP) {
              addLog(`[${act}] SKIP — APR gap too small: ${worst.name} (${worst.apr.toFixed(1)}%) → ${best.name} (${best.apr.toFixed(1)}%) = ${aprGap.toFixed(1)}% gap (need ${MIN_ROTATION_APR_GAP}%)`, "info");
              continue;
            }
            // Check 2: Will the extra yield from better APR cover rotation gas over ~1 hour?
            const moveAmt = Math.min(worst.stake, posSize);
            const hourlyYieldGain = moveAmt * (aprGap / 100) / (365 * 24); // extra MOTO/hour from better APR
            const rotationCheck = isProfitable("rotate", hourlyYieldGain, effectiveFeeRate);
            if (!rotationCheck.profitable) {
              addLog(`[${act}] SKIP — rotation gas (${rotationCheck.gasCost} sats) > 1hr yield gain (${Math.floor(rotationCheck.yieldSats)} sats) for ${aprGap.toFixed(1)}% APR gap`, "warning");
              continue;
            }
            addLog(`[${act}] Rotating: ${worst.name} (${worst.apr.toFixed(1)}%) → ${best.name} (${best.apr.toFixed(1)}%) | Gap: ${aprGap.toFixed(1)}% | Gas: ~${rotationCheck.gasCost} sats | Payback: ~${(rotationCheck.gasCost / (hourlyYieldGain * MOTO_PRICE_SATS)).toFixed(1)}hrs`, "info");
            sendNotification("rotation", `Rotating: ${worst.name} (${worst.apr.toFixed(1)}%) → ${best.name} (${best.apr.toFixed(1)}%) | Gap: ${aprGap.toFixed(1)}%`);
            if (strategy === "aggressive") {
              ...
              addLog(`[${act}] Moved ${fmtBtc(moveAmt)} | Fee: ~${rotationCheck.gasCost} sats`, "success");
            }
          }

        } else if (act === "Harvest rewards" && staked.length > 0) {
          const target = staked[Math.floor(Math.random() * staked.length)];
          // Profitability check: are accumulated rewards worth harvesting?
          const pendingRewards = target.rew;
          if (pendingRewards < MIN_HARVEST_MOTO) {
            addLog(`[${act}] SKIP ${target.name} — only ${pendingRewards.toFixed(2)} MOTO pending (need ${MIN_HARVEST_MOTO} MOTO to justify gas)`, "info");
            continue;
          }
          const harvestCheck = isProfitable("claim", pendingRewards, effectiveFeeRate);
          if (!harvestCheck.profitable) {
            addLog(`[${act}] SKIP ${target.name} — gas: ${harvestCheck.gasCost} sats > reward value: ${Math.floor(harvestCheck.yieldSats)} sats (wait for more rewards)`, "warning");
            continue;
          }
          const harvested = pendingRewards > 0 ? pendingRewards : rand(1, strategy === "aggressive" ? 30 : 15);
          ...
          // Clear rewards on harvested pool
          setPools(prev => prev.map(p => p.id === target.id ? { ...p, rew: 0 } : p));

          addLog(`[${act}] Harvested ${userReward.toFixed(4)} MOTO from ${target.name} | Gas: ~${harvestCheck.gasCost} sats | Net profit: ${Math.floor(harvestCheck.yieldSats - harvestCheck.gasCost)} sats`, "success");
          sendNotification("harvest", `Harvested ${userReward.toFixed(4)} MOTO from ${target.name} | Net: ${Math.floor(harvestCheck.yieldSats - harvestCheck.gasCost)} sats`);
          ...
          // Profitability: rebalancing is only worth it if imbalance is significant
          const maxDeviation = Math.max(...staked.map(p => Math.abs(p.stake - targetPer / 1e8)));
          const rebalGas = estimateGasCost("rebalance", effectiveFeeRate) * staked.length;
          if (maxDeviation < posSize * 0.2) {
            addLog(`[${act}] SKIP — positions already balanced (max deviation: ${(maxDeviation * 100 / (targetPer || 1)).toFixed(0)}%)`, "info");
            continue;
          }
          addLog(`[${act}] Rebalancing ${staked.length} positions to ~${fmtBtc(targetPer)} each | Est. gas: ~${rebalGas} sats`, "info");
          ...
          const gasEst = estimateGasCost("stake", effectiveFeeRate);
          addLog(`[${act}] Best: ${best?.name} at ${best?.apr.toFixed(1)}% APR | Current gas: ${gasEst} sats/tx | Min harvest: ${MIN_HARVEST_MOTO} MOTO`, "info");

        } else if (act === "Gas optimization") {
          const stakeGas = estimateGasCost("stake", effectiveFeeRate);
          const claimGas = estimateGasCost("claim", effectiveFeeRate);
          const rotateGas = estimateGasCost("rotate", effectiveFeeRate);
          const optimalTime = currentFeeLevel === "low" ? "NOW — optimal" : currentFeeLevel === "normal" ? "acceptable" : "wait for lower fees";
          addLog(`[${act}] Fee: ${feeRate} sat/vB (${currentFeeLevel}) | Stake: ${stakeGas}s Claim: ${claimGas}s Rotate: ${rotateGas}s | ${optimalTime}`, "info");
  ...
  }, [botRunning, walletConnected, pools, autoCompound, addLog, sendNotification, strategy, slippage, posSize, network, gasOpt, blockHeight, prevBlockHeight, feeRate]);
  ...
  return page === "guide" ? (
    <div style={{ minHeight:"100vh",background:"#050505",color:"#e0e0e0",fontFamily:"'DM Sans',sans-serif",padding:"60px 28px",maxWidth:720,margin:"0 auto" }}>
      ...
      <h1 style={{ fontSize:32,fontWeight:900,color:"#ff9800",marginBottom:6 }}>OrangeFarmer Guide</h1>
      <p style={{ fontSize:14,color:"#888",marginBottom:32 }}>Automated DeFi yield farming on Bitcoin Layer 1 via OPNet smart contracts</p>

      {/* How It Works */}
      <div style={{ marginBottom:32 }}>
        <h2 style={{ fontSize:18,fontWeight:800,color:"#fff",marginBottom:16,paddingBottom:8,borderBottom:"1px solid #1a1a1a" }}>How It Works</h2>
        <p style={{ fontSize:13,color:"#aaa",lineHeight:1.7,marginBottom:16 }}>
          OrangeFarmer is an automation layer — not a custodian. The bot monitors DeFi pools on OPNet, calculates optimal actions based on your strategy, and builds transactions for you. But it never holds your funds and can never move them without your explicit approval.
        </p>
        <p style={{ fontSize:13,color:"#aaa",lineHeight:1.7,marginBottom:16 }}>
          When the bot decides to deposit, harvest, or rotate between pools, it constructs a smart contract call (like <span style={{ color:"#ff9800",fontFamily:"'JetBrains Mono',monospace",fontSize:12 }}>stake(amount)</span> or <span style={{ color:"#ff9800",fontFamily:"'JetBrains Mono',monospace",fontSize:12 }}>claimRewards()</span>) and sends it to your OP_WALLET for signing. Your wallet pops up, you review the transaction details, and you approve or reject. Only after your signature does anything happen on-chain.
        </p>
        <p style={{ fontSize:13,color:"#aaa",lineHeight:1.7,marginBottom:20 }}>
          Your tokens move directly from your wallet into the protocol's smart contract (MotoSwap pool, staking contract, NativeSwap). There is no vault, no intermediary, no third-party custody. The bot is a browser app — it runs in your tab, talks to OPNet RPC nodes, and delegates all signing to your wallet. Your private keys never leave OP_WALLET.
        </p>
      </div>

      {/* Your Control */}
      <div style={{ marginBottom:32 }}>
        <h2 style={{ fontSize:18,fontWeight:800,color:"#fff",marginBottom:16,paddingBottom:8,borderBottom:"1px solid #1a1a1a" }}>You Stay in Control</h2>
        {[
          ["Your keys, your funds", "Private keys stay in OP_WALLET. The bot has zero access to them. Every transaction requires your wallet signature."],
          ["Confirmation before every deposit", "Enable Confirm Deposits in Settings to review gas cost, expected yield, and payback time before each operation."],
          ["Confirmation before bot starts", "When you click Start Bot, a summary modal shows your strategy, position size, network, and all settings before execution begins."],
          ["Telegram & Discord notifications", "Connect your Telegram bot or Discord webhook in the Alerts tab. Get notified of every harvest, pool rotation, and error — so you know exactly what the bot is doing even when you're away."],
          ["Profitability engine", "The bot checks gas cost vs expected yield before every action. It won't harvest if rewards are too small, won't rotate pools unless the APR gap justifies the gas, and won't compound unless the math works. No unprofitable transactions."],
          ["Fee-aware execution", "During high Bitcoin fee periods, the bot automatically delays expensive operations and only executes urgent actions like harvesting accumulated rewards."],
          ["Manual override anytime", "Pause the bot instantly. Withdraw from any pool manually. Change strategy mid-run. The bot adapts to your decisions."],
        ].map(([title, desc], i) => (
          <div key={i} style={{ padding:"14px 18px",background:"#0a0a0e",border:"1px solid #141418",borderRadius:10,marginBottom:8 }}>
            <div style={{ fontWeight:700,color:"#ff9800",marginBottom:4,fontSize:13 }}>{title}</div>
      ...
      {/* Quick Start */}
      <div style={{ marginBottom:32 }}>
        <h2 style={{ fontSize:18,fontWeight:800,color:"#fff",marginBottom:16,paddingBottom:8,borderBottom:"1px solid #1a1a1a" }}>Quick Start</h2>
        {[
          ["Install OP_WALLET","Download from Chrome Web Store and create or import your Bitcoin wallet."],
          ["Connect Wallet","Click Connect OP_WALLET in the sidebar. The app detects your wallet and reads your address. Network stays on your manual selection."],
          ["Set Up Notifications","Go to Alerts tab. Paste your Telegram bot token + chat ID or Discord webhook URL. Hit Connect & Test to verify."],
          ["Choose Strategy","Pick Conservative (staking only, low risk), Balanced (staking + LP), or Aggressive (max APR, frequent rotation)."],
          ["Set Position Size","In Settings, choose the BTC amount per operation. Start small — you can increase later."],
          ["Review & Start Bot","Click Start Bot. A confirmation modal shows all your settings. Review and confirm. The bot begins scanning pools every 30 seconds."],
          ["Monitor","Watch the Logs tab for real-time decisions. Check Portfolio for P&L. You'll get Telegram/Discord alerts for harvests, rotations, and errors."],
        ].map(([t,d],i) => (
          <div key={i} style={{ padding:"14px 18px",background:"#0a0a0e",border:"1px solid #141418",borderRadius:10,marginBottom:8,display:"flex",gap:14,alignItems:"flex-start" }}>
            ...
            <div><div style={{ fontWeight:700,color:"#eee",marginBottom:2 }}>{t}</div><div style={{ fontSize:12,color:"#888",lineHeight:1.5 }}>{d}</div></div>
          </div>
        ))}
      </div>
      ...
      {/* Transaction Flow */}
      <div style={{ marginBottom:32 }}>
        <h2 style={{ fontSize:18,fontWeight:800,color:"#fff",marginBottom:16,paddingBottom:8,borderBottom:"1px solid #1a1a1a" }}>Transaction Flow</h2>
        {[
          ["Bot decides to act", "Based on strategy, APR data, and profitability checks"],
          ["Approval tx built", "increaseAllowance() — gives the contract permission to move your tokens"],
          ["Action tx built", "stake(), addLiquidity(), claimRewards(), or unstake()"],
          ["OP_WALLET prompts you", "Review amount, contract address, and gas fee"],
          ["You sign or reject", "Nothing happens without your approval"],
          ["Transaction broadcasts", "Signed tx goes to Bitcoin network via OPNet"],
          ["On-chain confirmation", "Tokens are in the protocol contract. Bot updates UI."],
        ].map(([t,d],i) => (
          <div key={i} style={{ display:"flex",gap:12,alignItems:"flex-start",marginBottom:12 }}>
            <div style={{ width:24,height:24,borderRadius:"50%",background:i===4?"#00e676":"#1a1a1a",border:`1px solid ${i===4?"#00e676":"#333"}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700,color:i===4?"#000":"#888",flexShrink:0 }}>{i+1}</div>
            <div><span style={{ fontWeight:600,color:"#ccc",fontSize:13 }}>{t}</span><span style={{ color:"#666",fontSize:12 }}> — {d}</span></div>
          </div>
        ))}
      </div>
      ...
      {/* Supported Protocols */}
      <div style={{ marginBottom:32 }}>
        <h2 style={{ fontSize:18,fontWeight:800,color:"#fff",marginBottom:16,paddingBottom:8,borderBottom:"1px solid #1a1a1a" }}>Supported Protocols</h2>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:8 }}>
          {[
            ["MotoSwap","AMM router for token↔token LP","#ff9800"],
            ["NativeSwap","BTC↔token DEX, 2-phase commit","#00e676"],
            ["MOTO Staking","Single-asset staking + rewards","#42a5f5"],
            ["MotoChef","Multi-pool yield farming","#ff6b2b"],
          ].map(([name,desc,color],i) => (
            <div key={i} style={{ padding:"12px 16px",background:"#0a0a0e",borderRadius:8,borderLeft:`3px solid ${color}` }}>
              <div style={{ fontWeight:700,color:"#eee",fontSize:13 }}>{name}</div>
      ...
      <div style={{ display:"flex",gap:10,marginTop:24 }}>
        <button onClick={() => setPage("app")} style={{ padding:"12px 30px",background:"linear-gradient(135deg,#ff9800,#e65100)",border:"none",borderRadius:8,color:"#000",fontWeight:700,fontSize:14,cursor:"pointer" }}>Launch App</button>
        <button onClick={() => setPage("landing")} style={{ padding:"12px 30px",background:"none",border:"1px solid #222",borderRadius:8,color:"#888",fontWeight:600,fontSize:14,cursor:"pointer" }}>Back to Home</button>
      </div>
    </div>
  ) : page === "landing" ? (
    <div style={L.root}>
    ...
    <div style={{ display:"flex", minHeight:"100vh", background: T.bg, color: T.text, fontFamily:"'DM Sans', -apple-system, sans-serif", fontSize:13 }}>
      ...
      <main style={{ ...A.main, marginLeft: isMobile ? 0 : undefined }}>
        ...
        <header style={{ ...A.header, background: T.headerBg, borderBottom: `1px solid ${T.border}` }}>
          ...
          {walletConnected && (
            <button onClick={() => { if (botRunning) { setBotRunning(false); addLog("Bot paused", "warning"); } else { setBotConfirmModal(true); } }}
        ...
        <div style={A.content}>
                ...
                {sdkAvailable ? (
                  <>
                    <button onClick={realClaimRewards} disabled={txPending} style={{ padding:"8px 18px", background:"linear-gradient(135deg, #00e676, #00c853)", border:"none", borderRadius:8, color:"#000", fontWeight:700, fontSize:12, cursor:"pointer", opacity:txPending?0.5:1 }}>
                      {txPending && txTarget === "Claim Rewards" ? txStatus || "..." : "Claim Rewards"}
                    </button>
                    <button onClick={() => realUnstake(posSize)} disabled={txPending} style={{ padding:"8px 18px", background:"none", border:"1px solid #ff9800", borderRadius:8, color:"#ff9800", fontWeight:600, fontSize:12, cursor:"pointer", opacity:txPending?0.5:1 }}>
                      {txPending && txTarget === "Unstake" ? txStatus || "..." : `Unstake ${fmtBtc(posSize)}`}
                    </button>
                    ...
                    {(() => { try { const c = SDK.getNetworkConfig(network); return !c?.staking ? <span style={{ fontSize:11, color:"#ff9800" }}>No contracts on {network} — use regtest</span> : null; } catch { return null; } })()}
                  </>
                ) : (
                  <>
                    <button onClick={() => { const stakingPool = pools.find(p => p.proto === "Staking" && p.stake > 0); if (stakingPool) { setPools(prev => prev.map(p => p.proto === "Staking" ? { ...p, rew: 0 } : p)); addLog(`[Sim] Claimed rewards from staking pools`, "success"); } else { addLog("[Sim] No staking positions to claim from", "warning"); } }} style={{ padding:"8px 18px", background:"linear-gradient(135deg, #00e676, #00c853)", border:"none", borderRadius:8, color:"#000", fontWeight:700, fontSize:12, cursor:"pointer" }}>
                      Claim Rewards (Sim)
                    </button>
                    <button onClick={() => { setPools(prev => prev.map(p => p.proto === "Staking" ? { ...p, stake: 0, rew: 0 } : p)); addLog("[Sim] Unstaked all staking positions", "success"); }} style={{ padding:"8px 18px", background:"none", border:"1px solid #ff9800", borderRadius:8, color:"#ff9800", fontWeight:600, fontSize:12, cursor:"pointer" }}>
                      Unstake All (Sim)
                    </button>
                    <span style={{ fontSize:11, color:"#ff9800" }}>Simulation mode — no SDK</span>
                  </>
                )}
              </div>
            )}
                      ...
                      <td style={A.td}>
                        {eligible ? (
                          <div style={{ display:"flex", gap:4, alignItems:"center" }}>
                            <button onClick={() => { if (confirmDeposits) { confirmDeposit(p); } else if (sdkAvailable && walletConnected) { realDeposit(p); } else { setPools(prev => prev.map(pp => pp.id===p.id ? { ...pp, stake: pp.stake+posSize } : pp)); addLog(`[Sim] Deposited ${fmtBtc(posSize)} into ${p.name}`, "success"); } }} disabled={txPending && txTarget===p.name} style={{ ...A.actBtn, opacity: txPending && txTarget===p.name ? 0.5 : 1 }}>{txPending && txTarget===p.name ? txStatus || "..." : p.stake>0?"Add More":"Deposit"}</button>
                            {p.stake > 0 && (
                              <button onClick={() => { if(sdkAvailable && walletConnected && p.proto === "Staking") { realUnstake(p.stake); } else { setPools(prev => prev.map(pp => pp.id===p.id ? { ...pp, stake: 0, rew: 0 } : pp)); addLog(`[Sim] Withdrew all from ${p.name}`, "success"); } }} disabled={txPending} style={{ padding:"4px 10px", background:"none", border:"1px solid #ff980060", borderRadius:6, color:"#ff9800", fontSize:10, cursor:"pointer", opacity:txPending?0.5:1 }}>Withdraw</button>
                            )}
          ...
          {tab === "bot" && <>
            <div style={{ display:"grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap:16 }}>
              <div style={A.card}>
                ...
                <button onClick={() => { if (botRunning) { setBotRunning(false); addLog("Bot paused","warning"); } else { setBotConfirmModal(true); } }} style={{ ...A.bigBtn, background:botRunning?"#1a0e00":"#ff9800", color:botRunning?"#ff9800":"#000", border:botRunning?"1px solid #ff980033":"none" }}>{botRunning?"\u23F8 Pause":"\u25B6 Start"}</button>
              </div>
              <div style={A.card}>
          ...
          {tab === "notify" && (
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 16 }}>
              <div style={{ ...A.card, background: T.card, borderColor: T.border }}>
                <div style={A.cardHead}><h2 style={{ ...A.cardTitle, color: T.text }}>Telegram</h2>{tgConnected && <span style={{ fontSize:10,color:"#00e676",fontWeight:700 }}>● CONNECTED</span>}</div>
                <div style={{ fontSize: 12, color: T.sub, marginBottom: 12 }}>Send bot alerts to a Telegram chat via Bot API</div>
                <div style={{ fontSize: 11, color: T.sub, marginBottom: 8, lineHeight: 1.5 }}>
                  1. Message <span style={{ color:"#ff9800",fontWeight:600 }}>@BotFather</span> on Telegram → /newbot → copy the token<br/>
                  2. Add your bot to a group or message it directly<br/>
                  3. Get chat ID from <span style={{ color:"#ff9800",fontWeight:600 }}>@userinfobot</span> or the API<br/>
                  4. Paste below as: <span style={{ color:"#ccc",fontFamily:"'JetBrains Mono',monospace",fontSize:10 }}>TOKEN/CHAT_ID</span>
                </div>
                <div style={{ marginBottom: 12 }}>
                  <input value={notifTg} onChange={e => { setNotifTg(e.target.value); setTgConnected(false); }} placeholder="123456:ABC-DEF1234ghIkl-zyx/-1001234567890" style={{ width: "100%", padding: "10px 12px", background: T.input, border: `1px solid ${tgConnected ? "#00e676" : T.border}`, borderRadius: 8, color: T.text, fontSize: 12, outline: "none", fontFamily: "'JetBrains Mono',monospace" }} />
                </div>
                <button onClick={async () => {
                  if (!notifTg) { addLog("Enter bot token and chat ID first", "warning"); return; }
                  const parts = notifTg.split("/").filter(Boolean);
                  if (parts.length < 2) { addLog("Invalid format. Use: BOT_TOKEN/CHAT_ID", "warning"); return; }
                  const token = parts[0], chatId = parts.slice(1).join("/");
                  addLog("Testing Telegram connection...", "info");
                  try {
                    const res = await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
                      body: JSON.stringify({ chat_id: chatId, text: "🚜 *OrangeFarmer* connected! You will receive bot alerts here.", parse_mode: "Markdown" }),
                    });
                    const data = await res.json();
                    if (data.ok) {
                      setTgConnected(true);
                      addLog(`Telegram connected! Test message sent to chat ${chatId.slice(0,8)}...`, "success");
                    } else {
                      setTgConnected(false);
                      addLog(`Telegram error: ${data.description || "Unknown error"}`, "warning");
                    }
                  } catch (e) {
                    setTgConnected(false);
                    addLog(`Telegram connection failed: ${e.message}`, "warning");
                  }
                }} style={{ ...A.connectBtn, width: "100%", opacity: notifTg ? 1 : 0.5, background: tgConnected ? "linear-gradient(135deg, #00e676, #00c853)" : "linear-gradient(135deg, #ff9800, #e65100)" }}>
                  {tgConnected ? "✓ Connected — Send Test" : "Connect & Test"}
                </button>
                {tgConnected && <div style={{ fontSize:10,color:"#00e676",marginTop:8 }}>A test message was sent to your Telegram. If you didn't receive it, check your token and chat ID.</div>}
              </div>

              <div style={{ ...A.card, background: T.card, borderColor: T.border }}>
                <div style={A.cardHead}><h2 style={{ ...A.cardTitle, color: T.text }}>Discord</h2>{dcConnected && <span style={{ fontSize:10,color:"#00e676",fontWeight:700 }}>● CONNECTED</span>}</div>
                <div style={{ fontSize: 12, color: T.sub, marginBottom: 12 }}>Send alerts via Discord webhook</div>
                <div style={{ fontSize: 11, color: T.sub, marginBottom: 8, lineHeight: 1.5 }}>
                  1. Open your Discord server → Settings → Integrations → Webhooks<br/>
                  2. Click <span style={{ color:"#ff9800",fontWeight:600 }}>New Webhook</span> → choose channel → Copy URL<br/>
                  3. Paste the full webhook URL below
                </div>
                <div style={{ marginBottom: 12 }}>
                  <input value={notifDc} onChange={e => { setNotifDc(e.target.value); setDcConnected(false); }} placeholder="https://discord.com/api/webhooks/123.../abc..." style={{ width: "100%", padding: "10px 12px", background: T.input, border: `1px solid ${dcConnected ? "#00e676" : T.border}`, borderRadius: 8, color: T.text, fontSize: 12, outline: "none", fontFamily: "'JetBrains Mono',monospace" }} />
                </div>
                <button onClick={async () => {
                  if (!notifDc) { addLog("Enter Discord webhook URL first", "warning"); return; }
                  if (!notifDc.includes("discord.com/api/webhooks")) { addLog("Invalid Discord webhook URL", "warning"); return; }
                  addLog("Testing Discord connection...", "info");
                  try {
                    const res = await fetch(notifDc, {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
                      body: JSON.stringify({ content: "🚜 **OrangeFarmer** connected! You will receive bot alerts here." }),
                    });
                    if (res.ok || res.status === 204) {
                      setDcConnected(true);
                      addLog("Discord connected! Test message sent to your channel.", "success");
                    } else {
                      setDcConnected(false);
                      const err = await res.text().catch(() => "");
                      addLog(`Discord error: HTTP ${res.status} ${err.slice(0,100)}`, "warning");
                    }
                  } catch (e) {
                    setDcConnected(false);
                    addLog(`Discord connection failed: ${e.message}`, "warning");
                  }
                }} style={{ ...A.connectBtn, width: "100%", opacity: notifDc ? 1 : 0.5, background: dcConnected ? "linear-gradient(135deg, #00e676, #00c853)" : "linear-gradient(135deg, #ff9800, #e65100)" }}>
                  {dcConnected ? "✓ Connected — Send Test" : "Connect & Test"}
                </button>
                {dcConnected && <div style={{ fontSize:10,color:"#00e676",marginTop:8 }}>A test message was sent to your Discord channel. Check the channel to confirm.</div>}
              </div>

              <div style={{ ...A.card, background: T.card, borderColor: T.border, gridColumn: isMobile ? "1" : "1 / -1" }}>
                <div style={A.cardHead}><h2 style={{ ...A.cardTitle, color: T.text }}>Alert Events</h2><span style={{ fontSize:10,color: (tgConnected || dcConnected) ? "#00e676" : "#555" }}>{tgConnected || dcConnected ? "Notifications active" : "Connect Telegram or Discord first"}</span></div>
          ...
          {tab === "settings" && (
            <div style={{ display:"grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap:16 }}>
              <div style={A.card}>
                ...
                {[["Auto-Compound","Reinvest rewards automatically",autoCompound,()=>setAutoCompound(!autoCompound)],["Gas Optimization","Batch transactions",gasOpt,()=>setGasOpt(!gasOpt)],["Confirm Deposits","Show confirmation with tx details before each deposit",confirmDeposits,()=>setConfirmDeposits(!confirmDeposits)]].map(([t,d,v,fn],i)=>(
                ...
                <div style={{ marginTop:20,padding:"14px 0",borderTop:"1px solid #111" }}>
                  <div style={{ fontWeight:700,marginBottom:12,color:"#ff9800",fontSize:13,letterSpacing:"0.05em" }}>PROFITABILITY ENGINE</div>
                  <div style={{ fontSize:11,color:"#555",marginBottom:14 }}>Gas-vs-yield checks for every action. Prevents unprofitable transactions on 10min BTC blocks.</div>
                  <div style={{ display:"grid",gap:12 }}>
                    <div>
                      <div style={{ fontSize:12,color:"#888",marginBottom:4 }}>Min Profit Ratio: <span style={{ color:"#ff9800",fontWeight:700 }}>{MIN_PROFIT_RATIO}x</span></div>
                      <div style={{ fontSize:10,color:"#555" }}>Only execute if yield ≥ {MIN_PROFIT_RATIO}x gas cost</div>
                    </div>
                    <div>
                      <div style={{ fontSize:12,color:"#888",marginBottom:4 }}>Min Harvest: <span style={{ color:"#ff9800",fontWeight:700 }}>{MIN_HARVEST_MOTO} MOTO</span></div>
                      <div style={{ fontSize:10,color:"#555" }}>Skip harvest if pending rewards below this</div>
                    </div>
                    <div>
                      <div style={{ fontSize:12,color:"#888",marginBottom:4 }}>Min Rotation Gap: <span style={{ color:"#ff9800",fontWeight:700 }}>{MIN_ROTATION_APR_GAP}%</span></div>
                      <div style={{ fontSize:10,color:"#555" }}>APR difference needed to justify pool switch</div>
                    </div>
                    <div>
                      <div style={{ fontSize:12,color:"#888",marginBottom:4 }}>Current Gas Estimates</div>
                      <div style={{ display:"flex",gap:6,flexWrap:"wrap",marginTop:4 }}>
                        {[["Stake",estimateGasCost("stake",Math.max(feeRate,1))],["Claim",estimateGasCost("claim",Math.max(feeRate,1))],["Compound",estimateGasCost("compound",Math.max(feeRate,1))],["Rotate",estimateGasCost("rotate",Math.max(feeRate,1))]].map(([label,cost])=>(
                          <div key={label} style={{ background:"#0a0a10",border:"1px solid #1a1a20",borderRadius:6,padding:"4px 8px",fontSize:10 }}>
                            <span style={{ color:"#555" }}>{label}: </span><span style={{ color:"#ff9800",fontWeight:600 }}>{cost}s</span>
                          </div>
                        ))}
                      </div>
                    </div>
      ...
      {botConfirmModal && (
        <div style={{ position:"fixed",inset:0,zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,0.85)",backdropFilter:"blur(8px)" }} onClick={() => setBotConfirmModal(false)}>
          <div onClick={e => e.stopPropagation()} style={{ background:"#0c0c12",border:"1px solid #1a1a24",borderRadius:16,padding:0,width:Math.min(460, typeof window !== "undefined" ? window.innerWidth - 32 : 460),boxShadow:"0 24px 80px rgba(0,0,0,0.6), 0 0 40px rgba(255,152,0,0.08)" }}>
            {/* Header */}
            <div style={{ padding:"20px 24px 16px",borderBottom:"1px solid #141418",display:"flex",justifyContent:"space-between",alignItems:"center" }}>
              <div style={{ fontSize:16,fontWeight:800,color:"#fff" }}>Start Bot</div>
              <button onClick={() => setBotConfirmModal(false)} style={{ background:"none",border:"none",color:"#555",fontSize:22,cursor:"pointer",padding:4,lineHeight:1 }}>×</button>
            </div>

            {/* Bot config summary */}
            <div style={{ padding:"16px 24px" }}>
              <div style={{ fontSize:12,color:"#888",marginBottom:14 }}>The bot will run with these settings:</div>
              {[
                ["Strategy", STRATEGIES.find(s => s.id === strategy)?.name || strategy, "#ff9800"],
                ["Position Size", fmtBtc(posSize), "#fff"],
                ["Network", network, "#ccc"],
                ["Scan Interval", "30 seconds", "#ccc"],
                ["Auto-Compound", autoCompound ? "ON" : "OFF", autoCompound ? "#00e676" : "#ff3d00"],
                ["Gas Optimization", gasOpt ? "ON" : "OFF", gasOpt ? "#00e676" : "#ff3d00"],
                ["Confirm Deposits", confirmDeposits ? "ON" : "OFF", confirmDeposits ? "#00e676" : "#ff3d00"],
                ["Slippage Tolerance", slippage + "%", "#ccc"],
                ["Fee Rate", feeRate > 0 ? feeRate + " sat/vB" : "auto", "#ccc"],
                ["Eligible Pools", pools.filter(p => poolAvailable(p, network) && riskAllowed(p.risk, STRATEGIES.find(s=>s.id===strategy)?.maxRisk)).length + " / " + pools.length, "#ccc"],
                ["Wallet", walletConnected ? shortenAddr(walletAddr) : "Not connected (simulation)", walletConnected ? "#00e676" : "#ff9800"],
                ["Mode", sdkAvailable && walletConnected ? "LIVE — real transactions" : "SIMULATION", sdkAvailable && walletConnected ? "#00e676" : "#ff9800"],
              ].map(([label, value, color], i) => (
                <div key={i} style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0",borderBottom:i < 11 ? "1px solid #0a0a0e" : "none" }}>
            ...
            {/* Profitability info */}
            <div style={{ margin:"0 24px 16px",padding:"10px 14px",background:"rgba(255,152,0,0.06)",border:"1px solid rgba(255,152,0,0.15)",borderRadius:8 }}>
              <div style={{ fontSize:11,color:"#ff9800",fontWeight:600 }}>Profitability Engine Active</div>
              <div style={{ fontSize:10,color:"#aa7733",marginTop:4 }}>Actions only execute when yield exceeds gas cost by {MIN_PROFIT_RATIO}x. Harvest minimum: {MIN_HARVEST_MOTO} MOTO. Rotation minimum APR gap: {MIN_ROTATION_APR_GAP}%.</div>
            </div>

            {/* Buttons */}
            <div style={{ padding:"12px 24px 20px",display:"flex",gap:10 }}>
              <button onClick={() => setBotConfirmModal(false)} style={{ flex:1,padding:"12px",background:"none",border:"1px solid #222",borderRadius:10,color:"#888",fontSize:13,fontWeight:700,cursor:"pointer" }}>
                Cancel
              </button>
              <button onClick={() => { setBotConfirmModal(false); setBotRunning(true); addLog("Bot started — monitoring every 30s", "success"); }} style={{ flex:2,padding:"12px",background:"linear-gradient(135deg, #ff9800, #e65100)",border:"none",borderRadius:10,color:"#000",fontSize:13,fontWeight:800,cursor:"pointer",letterSpacing:"0.03em" }}>
                Confirm & Start Bot
              </button>
            </div>
          </div>
      ...
      {/* ═══ Deposit Confirmation Modal ═══ */}
      {confirmModal && (
        <div style={{ position:"fixed",inset:0,zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,0.85)",backdropFilter:"blur(8px)" }} onClick={() => setConfirmModal(null)}>
          <div onClick={e => e.stopPropagation()} style={{ background:"#0c0c12",border:"1px solid #1a1a24",borderRadius:16,padding:0,width:Math.min(480, typeof window !== "undefined" ? window.innerWidth - 32 : 480),maxHeight:"90vh",overflow:"auto",boxShadow:"0 24px 80px rgba(0,0,0,0.6), 0 0 40px rgba(255,152,0,0.08)" }}>
            {/* Header */}
            <div style={{ padding:"20px 24px 16px",borderBottom:"1px solid #141418",display:"flex",justifyContent:"space-between",alignItems:"center" }}>
              <div>
                <div style={{ fontSize:16,fontWeight:800,color:"#fff" }}>Confirm Deposit</div>
                <div style={{ fontSize:11,color:confirmModal.isSim ? "#ff9800" : "#00e676",marginTop:2,fontWeight:600 }}>
                  {confirmModal.isSim ? "SIMULATION MODE" : "LIVE TRANSACTION"}
                </div>
              </div>
              <button onClick={() => setConfirmModal(null)} style={{ background:"none",border:"none",color:"#555",fontSize:22,cursor:"pointer",padding:4,lineHeight:1 }}>×</button>
            </div>

            {/* Pool info */}
            <div style={{ padding:"16px 24px",borderBottom:"1px solid #0e0e14" }}>
              <div style={{ display:"flex",alignItems:"center",gap:12,marginBottom:12 }}>
                <div style={{ width:40,height:40,borderRadius:10,background:`hsl(${confirmModal.pool.id*47},65%,40%)`,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:16,color:"#fff" }}>{confirmModal.pool.t0[0]}</div>
                <div>
                  <div style={{ fontWeight:700,fontSize:15 }}>{confirmModal.pool.name}</div>
                  <div style={{ fontSize:11,color:"#666" }}>{confirmModal.pool.proto} • {confirmModal.depositType}</div>
                </div>
                <div style={{ marginLeft:"auto",textAlign:"right" }}>
                  <div style={{ fontSize:20,fontWeight:800,color:"#ff9800" }}>{confirmModal.pool.apr.toFixed(1)}%</div>
            ...
            <div style={{ padding:"16px 24px" }}>
              {[
                ["Deposit Amount", fmtBtc(confirmModal.amount), "#fff"],
                ["Pool", `${confirmModal.pool.t0}${confirmModal.pool.t1 ? " / " + confirmModal.pool.t1 : ""}`, "#ccc"],
                ["Network", confirmModal.network, "#ccc"],
                ["Wallet", confirmModal.walletAddr.length > 20 ? shortenAddr(confirmModal.walletAddr) : confirmModal.walletAddr, "#888"],
                ["Est. Gas Cost", `${confirmModal.gasCost.toLocaleString()} sats`, confirmModal.gasCost > 500 ? "#ff3d00" : "#00e676"],
                ["Fee Rate", `${confirmModal.feeRateUsed} sat/vB`, "#ccc"],
                ["Daily Yield Est.", `${confirmModal.dailyYield.toFixed(4)} MOTO (~${Math.floor(confirmModal.dailyYieldSats)} sats)`, "#00e676"],
                ["Gas Payback", confirmModal.daysToPaybackGas > 0 ? `~${confirmModal.daysToPaybackGas.toFixed(1)} days` : "instant", confirmModal.daysToPaybackGas > 7 ? "#ff3d00" : confirmModal.daysToPaybackGas > 3 ? "#ff9800" : "#00e676"],
              ].map(([label, value, color], i) => (
                <div key={i} style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:i < 7 ? "1px solid #0a0a0e" : "none" }}>
                  <span style={{ fontSize:12,color:"#666" }}>{label}</span>
            ...
            {/* Profitability warning if bad deal */}
            {confirmModal.daysToPaybackGas > 7 && (
              <div style={{ margin:"0 24px 16px",padding:"10px 14px",background:"rgba(255,61,0,0.08)",border:"1px solid rgba(255,61,0,0.2)",borderRadius:8 }}>
                <div style={{ fontSize:12,color:"#ff3d00",fontWeight:700 }}>⚠ Slow payback</div>
                <div style={{ fontSize:11,color:"#ff6b3d",marginTop:4 }}>Gas cost will take ~{confirmModal.daysToPaybackGas.toFixed(0)} days to recover from yield. Consider waiting for lower fees or a larger deposit.</div>
              </div>
            )}

            {/* Action buttons */}
            <div style={{ padding:"16px 24px 20px",display:"flex",gap:10 }}>
              <button onClick={() => setConfirmModal(null)} style={{ flex:1,padding:"12px",background:"none",border:"1px solid #222",borderRadius:10,color:"#888",fontSize:13,fontWeight:700,cursor:"pointer" }}>
                Cancel
              </button>
              <button onClick={executeConfirmedDeposit} style={{ flex:2,padding:"12px",background:"linear-gradient(135deg, #ff9800, #e65100)",border:"none",borderRadius:10,color:"#000",fontSize:13,fontWeight:800,cursor:"pointer",letterSpacing:"0.03em" }}>
                {confirmModal.isSim ? `Deposit (Sim) ${fmtBtc(confirmModal.amount)}` : `Confirm Deposit ${fmtBtc(confirmModal.amount)}`}
              </button>
            </div>
          </div>
        </div>
